"use client";

import { CreateboardModal } from "@/components/modal/createBoardModal";
import { useState } from "react";

const recentBoards = [
  {
    name: "Flowora Backend",
    workspace: "Engineering",
    progress: 72,
    members: ["P", "RK", "AS"],
    memberColors: ["bg-brand", "bg-emerald-400", "bg-pink-400"],
    status: "active",
    lastViewed: "Just now",
    taskCount: 18,
  },
  {
    name: "MediQ Frontend",
    workspace: "Engineering",
    progress: 45,
    members: ["P", "MJ"],
    memberColors: ["bg-brand", "bg-purple-400"],
    status: "active",
    lastViewed: "1h ago",
    taskCount: 11,
  },
  {
    name: "Design System v2",
    workspace: "Design Team",
    progress: 88,
    members: ["AS", "RK", "MJ", "P"],
    memberColors: ["bg-pink-400", "bg-emerald-400", "bg-purple-400", "bg-brand"],
    status: "review",
    lastViewed: "Yesterday",
    taskCount: 6,
  },
  {
    name: "Q2 Marketing Push",
    workspace: "Marketing",
    progress: 30,
    members: ["MJ", "RK"],
    memberColors: ["bg-purple-400", "bg-emerald-400"],
    status: "active",
    lastViewed: "2 days ago",
    taskCount: 22,
  },
];

const statusConfig: Record<string, { label: string; color: string; dot: string }> = {
  active: { label: "Active", color: "text-emerald-600 bg-emerald-50", dot: "bg-emerald-400" },
  review: { label: "In Review", color: "text-amber-600 bg-amber-50", dot: "bg-amber-400" },
  paused: { label: "Paused", color: "text-slate-500 bg-slate-100", dot: "bg-slate-400" },
};

export default function RightPanel() {

  const [isModalOpen, setIsModalOpen] = useState(false);

  return (
    <aside className="w-72 shrink-0 hidden lg:flex flex-col gap-4 p-4 h-[calc(100vh-56px)] sticky top-14 overflow-y-auto">
      {/* Create new board CTA */}
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-brand to-brand-dark p-5 text-white">
        <div className="absolute -top-4 -right-4 w-24 h-24 bg-white/5 rounded-full" />
        <div className="absolute -bottom-6 -right-2 w-16 h-16 bg-white/5 rounded-full" />
        <p className="text-xs font-semibold text-blue-100 uppercase tracking-wider mb-1">New Board</p>
        <h3 className="text-base font-semibold leading-snug mb-3">Start a fresh project board</h3>
        <button onClick={()=> setIsModalOpen(true)} className="flex items-center gap-2 bg-white text-brand text-sm font-semibold px-4 py-2 rounded-lg hover:bg-blue-50 transition-colors shadow-sm">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
          </svg>
          Create Board
        </button>
      </div>

      {/* Recently Viewed */}
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden flex-1">
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-slate-50">
          <h2 className="text-sm font-semibold text-slate-700">Recent Boards</h2>
          <button className="text-xs text-brand hover:text-brand-dark font-medium">All projects</button>
        </div>

        <div className="divide-y divide-slate-50">
          {recentBoards.map((board) => {
            const sc = statusConfig[board.status];
            return (
              <div
                key={board.name}
                className="px-4 py-3.5 hover:bg-slate-50/60 transition-colors cursor-pointer group"
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div>
                    <p className="text-sm font-medium text-slate-800 group-hover:text-brand transition-colors leading-snug">
                      {board.name}
                    </p>
                    <p className="text-[11px] text-slate-400 mt-0.5">{board.workspace}</p>
                  </div>
                  <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5 ${sc.color}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${sc.dot}`} />
                    {sc.label}
                  </span>
                </div>

                {/* Progress bar */}
                <div className="flex items-center gap-2 mb-2.5">
                  <div className="flex-1 h-1 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-brand to-brand-mid rounded-full transition-all"
                      style={{ width: `${board.progress}%` }}
                    />
                  </div>
                  <span className="text-[10px] text-slate-400 font-medium tabular-nums">{board.progress}%</span>
                </div>

                <div className="flex items-center justify-between">
                  {/* Stacked avatars */}
                  <div className="flex -space-x-1.5">
                    {board.members.slice(0, 3).map((m, i) => (
                      <div
                        key={i}
                        className={`w-5 h-5 rounded-full ${board.memberColors[i]} text-white text-[8px] font-bold flex items-center justify-center ring-1 ring-white`}
                      >
                        {m}
                      </div>
                    ))}
                    {board.members.length > 3 && (
                      <div className="w-5 h-5 rounded-full bg-slate-200 text-slate-500 text-[8px] font-bold flex items-center justify-center ring-1 ring-white">
                        +{board.members.length - 3}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-1 text-[11px] text-slate-400">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    {board.lastViewed}
                  </div>
                </div>
              </div>
            );
          })}
        </div>

        <div className="p-4 border-t border-slate-50">
          <button className="w-full flex items-center justify-center gap-2 py-2 rounded-lg border border-dashed border-slate-200 text-sm text-slate-400 hover:text-brand hover:border-brand hover:bg-brand-light/30 transition-all">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            New project
          </button>
        </div>
      </div>
      < CreateboardModal  isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}  />
    </aside>
  );
}